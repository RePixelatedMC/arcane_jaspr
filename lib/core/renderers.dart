import 'package:jaspr/jaspr.dart';

import 'props/accordion_props.dart';
import 'props/alert_banner_props.dart';
import 'props/alert_props.dart';
import 'props/animated_counter_props.dart';
import 'props/arrow_link_props.dart';
import 'props/aspect_ratio_props.dart';
import 'props/auth_branding_panel_props.dart';
import 'props/auth_layout_props.dart';
import 'props/auth_split_layout_props.dart';
import 'props/author_card_props.dart';
import 'props/avatar_props.dart';
import 'props/badge_props.dart';
import 'props/bar_props.dart';
import 'props/bottom_navigation_props.dart';
import 'props/breadcrumbs_props.dart';
import 'props/button_panel_props.dart';
import 'props/button_props.dart';
import 'props/calendar_props.dart';
import 'props/callout_props.dart';
import 'props/card_props.dart';
import 'props/card_section_props.dart';
import 'props/carpet_props.dart';
import 'props/center_body_props.dart';
import 'props/check_list_props.dart';
import 'props/checkbox_props.dart';
import 'props/chip_props.dart';
import 'props/code_snippet_props.dart';
import 'props/code_window_props.dart';
import 'props/color_input_props.dart';
import 'props/command_props.dart';
import 'props/confirm_dialog_props.dart';
import 'props/context_menu_props.dart';
import 'props/cta_banner_props.dart';
import 'props/cycle_button_props.dart';
import 'props/dashboard_layout_props.dart';
import 'props/data_table_props.dart';
import 'props/date_picker_props.dart';
import 'props/dialog_props.dart';
import 'props/disclosure_props.dart';
import 'props/dot_indicator_props.dart';
import 'props/drawer_props.dart';
import 'props/dropdown_menu_props.dart';
import 'props/empty_state_props.dart';
import 'props/expander_props.dart';
import 'props/fab_props.dart';
import 'props/fade_edge_props.dart';
import 'props/feature_card_props.dart';
import 'props/feature_showcase_props.dart';
import 'props/field_wrapper_props.dart';
import 'props/file_upload_props.dart';
import 'props/flexi_cards_props.dart';
import 'props/floating_props.dart';
import 'props/flow_props.dart';
import 'props/footer_column_props.dart';
import 'props/footer_props.dart';
import 'props/game_tile_props.dart';
import 'props/glass_props.dart';
import 'props/gradient_text_props.dart';
import 'props/gutter_props.dart';
import 'props/header_props.dart';
import 'props/hero_section_props.dart';
import 'props/icon_button_props.dart';
import 'props/integration_card_props.dart';
import 'props/kbd_props.dart';
import 'props/loader_props.dart';
import 'props/logo_carousel_props.dart';
import 'props/marquee_props.dart';
import 'props/menubar_props.dart';
import 'props/meter_props.dart';
import 'props/mobile_menu_props.dart';
import 'props/newsletter_form_props.dart';
import 'props/number_input_props.dart';
import 'props/otp_input_props.dart';
import 'props/pagination_props.dart';
import 'props/placeholder_image_props.dart';
import 'props/pricing_card_props.dart';
import 'props/progress_props.dart';
import 'props/promo_props.dart';
import 'props/radio_cards_props.dart';
import 'props/radio_group_props.dart';
import 'props/rating_stars_props.dart';
import 'props/resizable_props.dart';
import 'props/scroll_area_props.dart';
import 'props/scroll_rail_props.dart';
import 'props/search_props.dart';
import 'props/section_header_props.dart';
import 'props/section_props.dart';
import 'props/select_props.dart';
import 'props/separator_props.dart';
import 'props/settings_section_props.dart';
import 'props/sidebar_props.dart';
import 'props/skeleton_props.dart';
import 'props/slider_props.dart';
import 'props/slot_counter_props.dart';
import 'props/social_icons_props.dart';
import 'props/stat_card_props.dart';
import 'props/stat_display_props.dart';
import 'props/static_table_props.dart';
import 'props/status_badge_props.dart';
import 'props/status_indicator_props.dart';
import 'props/stepper_props.dart';
import 'props/surface_card_props.dart';
import 'props/switcher_props.dart';
import 'props/tabs_props.dart';
import 'props/tag_input_props.dart';
import 'props/testimonial_card_props.dart';
import 'props/text_input_props.dart';
import 'props/tile_props.dart';
import 'props/time_dialog_props.dart';
import 'props/time_picker_props.dart';
import 'props/timeline_props.dart';
import 'props/toast_props.dart';
import 'props/toggle_group_props.dart';
import 'props/toggle_switch_props.dart';
import 'props/tracker_props.dart';
import 'props/tree_view_props.dart';

export 'props/accordion_props.dart';
export 'props/alert_banner_props.dart';
export 'props/alert_props.dart';
export 'props/animated_counter_props.dart';
export 'props/arrow_link_props.dart';
export 'props/aspect_ratio_props.dart';
export 'props/auth_branding_panel_props.dart';
export 'props/auth_layout_props.dart';
export 'props/auth_split_layout_props.dart';
export 'props/author_card_props.dart';
export 'props/avatar_props.dart';
export 'props/badge_props.dart';
export 'props/bar_props.dart';
export 'props/bottom_navigation_props.dart';
export 'props/breadcrumbs_props.dart';
export 'props/button_panel_props.dart';
export 'props/button_props.dart';
export 'props/calendar_props.dart';
export 'props/callout_props.dart';
export 'props/card_props.dart';
export 'props/card_section_props.dart';
export 'props/carpet_props.dart';
export 'props/center_body_props.dart';
export 'props/check_list_props.dart';
export 'props/checkbox_props.dart';
export 'props/chip_props.dart';
export 'props/code_snippet_props.dart';
export 'props/code_window_props.dart';
export 'props/color_input_props.dart';
export 'props/command_props.dart';
export 'props/confirm_dialog_props.dart';
export 'props/context_menu_props.dart';
export 'props/cta_banner_props.dart';
export 'props/cycle_button_props.dart';
export 'props/dashboard_layout_props.dart';
export 'props/data_table_props.dart';
export 'props/date_picker_props.dart';
export 'props/dialog_props.dart';
export 'props/disclosure_props.dart';
export 'props/dot_indicator_props.dart';
export 'props/drawer_props.dart';
export 'props/dropdown_menu_props.dart';
export 'props/empty_state_props.dart';
export 'props/expander_props.dart';
export 'props/fab_props.dart';
export 'props/fade_edge_props.dart';
export 'props/feature_card_props.dart';
export 'props/feature_showcase_props.dart';
export 'props/field_wrapper_props.dart';
export 'props/file_upload_props.dart';
export 'props/flexi_cards_props.dart';
export 'props/floating_props.dart';
export 'props/flow_props.dart';
export 'props/footer_column_props.dart';
export 'props/footer_props.dart';
export 'props/game_tile_props.dart';
export 'props/glass_props.dart';
export 'props/gradient_text_props.dart';
export 'props/gutter_props.dart';
export 'props/header_props.dart';
export 'props/hero_section_props.dart';
export 'props/icon_button_props.dart';
export 'props/integration_card_props.dart';
export 'props/kbd_props.dart';
export 'props/loader_props.dart';
export 'props/logo_carousel_props.dart';
export 'props/marquee_props.dart';
export 'props/menubar_props.dart';
export 'props/meter_props.dart';
export 'props/mobile_menu_props.dart';
export 'props/newsletter_form_props.dart';
export 'props/number_input_props.dart';
export 'props/otp_input_props.dart';
export 'props/pagination_props.dart';
export 'props/placeholder_image_props.dart';
export 'props/pricing_card_props.dart';
export 'props/progress_props.dart';
export 'props/promo_props.dart';
export 'props/radio_cards_props.dart';
export 'props/radio_group_props.dart';
export 'props/rating_stars_props.dart';
export 'props/resizable_props.dart';
export 'props/scroll_area_props.dart';
export 'props/scroll_rail_props.dart';
export 'props/search_props.dart';
export 'props/section_header_props.dart';
export 'props/section_props.dart';
export 'props/select_props.dart';
export 'props/separator_props.dart';
export 'props/settings_section_props.dart';
export 'props/sidebar_props.dart';
export 'props/skeleton_props.dart';
export 'props/slider_props.dart';
export 'props/slot_counter_props.dart';
export 'props/social_icons_props.dart';
export 'props/stat_card_props.dart';
export 'props/stat_display_props.dart';
export 'props/static_table_props.dart';
export 'props/status_badge_props.dart';
export 'props/status_indicator_props.dart';
export 'props/stepper_props.dart';
export 'props/surface_card_props.dart';
export 'props/switcher_props.dart';
export 'props/tabs_props.dart';
export 'props/tag_input_props.dart';
export 'props/testimonial_card_props.dart';
export 'props/text_input_props.dart';
export 'props/tile_props.dart';
export 'props/time_dialog_props.dart';
export 'props/time_picker_props.dart';
export 'props/timeline_props.dart';
export 'props/toast_props.dart';
export 'props/toggle_group_props.dart';
export 'props/toggle_switch_props.dart';
export 'props/tracker_props.dart';
export 'props/tree_view_props.dart';

/// Interface that all stylesheets must implement.
///
/// This abstract class composes all renderer contract mixins from props files.
/// Each mixin defines the renderer method signatures for its component type.
abstract class ComponentRenderers
    with
        AccordionRendererContract,
        AlertBannerRendererContract,
        AlertRendererContract,
        AnimatedCounterRendererContract,
        ArrowLinkRendererContract,
        AspectRatioRendererContract,
        AuthBrandingPanelRendererContract,
        AuthLayoutRendererContract,
        AuthorCardRendererContract,
        AuthSplitLayoutRendererContract,
        AvatarRendererContract,
        BadgeRendererContract,
        BarRendererContract,
        BottomNavigationRendererContract,
        BreadcrumbsRendererContract,
        ButtonPanelRendererContract,
        ButtonRendererContract,
        CalendarRendererContract,
        CalloutRendererContract,
        CardRendererContract,
        CardSectionRendererContract,
        CarpetRendererContract,
        CenterBodyRendererContract,
        CheckboxRendererContract,
        CheckListRendererContract,
        ChipRendererContract,
        CodeSnippetRendererContract,
        CodeWindowRendererContract,
        ColorInputRendererContract,
        CommandRendererContract,
        ConfirmDialogRendererContract,
        ContextMenuRendererContract,
        CtaBannerRendererContract,
        CycleButtonRendererContract,
        DashboardLayoutRendererContract,
        DataTableRendererContract,
        DatePickerRendererContract,
        DialogRendererContract,
        DisclosureRendererContract,
        DotIndicatorRendererContract,
        DrawerRendererContract,
        DropdownMenuRendererContract,
        EmptyStateRendererContract,
        ExpanderRendererContract,
        FabRendererContract,
        FadeEdgeRendererContract,
        FeatureCardRendererContract,
        FeatureShowcaseRendererContract,
        FieldWrapperRendererContract,
        FileUploadRendererContract,
        FlexiCardsRendererContract,
        FloatingRendererContract,
        FlowRendererContract,
        FooterColumnRendererContract,
        FooterRendererContract,
        GameTileRendererContract,
        GlassRendererContract,
        GradientTextRendererContract,
        GutterRendererContract,
        HeaderRendererContract,
        HeroSectionRendererContract,
        IconButtonRendererContract,
        IntegrationCardRendererContract,
        KbdRendererContract,
        LoaderRendererContract,
        LogoCarouselRendererContract,
        MarqueeRendererContract,
        MenubarRendererContract,
        MeterRendererContract,
        MobileMenuRendererContract,
        NewsletterFormRendererContract,
        NumberInputRendererContract,
        OtpInputRendererContract,
        PaginationRendererContract,
        PlaceholderImageRendererContract,
        PricingCardRendererContract,
        ProgressRendererContract,
        PromoRendererContract,
        RadioCardsRendererContract,
        RadioGroupRendererContract,
        RatingStarsRendererContract,
        ResizableRendererContract,
        ScrollAreaRendererContract,
        ScrollRailRendererContract,
        SearchRendererContract,
        SectionHeaderRendererContract,
        SectionRendererContract,
        SelectRendererContract,
        SeparatorRendererContract,
        SettingsSectionRendererContract,
        SidebarRendererContract,
        SkeletonRendererContract,
        SliderRendererContract,
        SlotCounterRendererContract,
        SocialIconsRendererContract,
        StatCardRendererContract,
        StatDisplayRendererContract,
        StaticTableRendererContract,
        StatusBadgeRendererContract,
        StatusIndicatorRendererContract,
        StepperRendererContract,
        SurfaceCardRendererContract,
        SwitcherRendererContract,
        TabsRendererContract,
        TagInputRendererContract,
        TestimonialCardRendererContract,
        TextInputRendererContract,
        TileRendererContract,
        TimeDialogRendererContract,
        TimelineRendererContract,
        TimePickerRendererContract,
        ToastRendererContract,
        ToggleGroupRendererContract,
        ToggleSwitchRendererContract,
        TrackerRendererContract,
        TreeViewRendererContract {
  const ComponentRenderers();
}
