...
/// Codex Breadcrumbs renderer.
///
/// Implements the Codex design language:
/// - Larger size options
/// - Accent-colored links
/// - More spacing between items
class CodexBreadcrumbs extends StatelessComponent {
  final BreadcrumbsProps props;

  const CodexBreadcrumbs(this.props, {super.key});

  @override
  Component build(BuildContext context) {
    // Codex size-specific styles
    final (String fontSize, String gap) = switch (props.size) {
      BreadcrumbSizeVariant.sm => ('0.75rem', '0.5rem'),
      BreadcrumbSizeVariant.md => ('0.875rem', '0.625rem'), // Codex: slightly more gap
      BreadcrumbSizeVariant.lg => ('1rem', '0.75rem'),
    };

    return dom.nav(
      classes: 'codex-breadcrumbs',
  ...
  Component _buildItem(BreadcrumbItemProps item, int index, bool isLast) {
    ...
    final List<Component> children = [
      if (index == 0 && props.showHomeIcon)
        ...
        item.icon!,
        const dom.span(
          styles: dom.Styles(raw: {'width': '0.375rem'}),
          [],
        ),
      ],
    ...
    if (props.onItemClick != null && !isLast) {
      return dom.button(
        classes: 'codex-breadcrumb-button',
        styles: const dom.Styles(raw: {
          'display': 'inline-flex',
          'align-items': 'center',
    ...
    return dom.span(
      classes: 'codex-breadcrumb-current',
      styles: dom.Styles(raw: {
        'display': 'inline-flex',
        'align-items': 'center',
  ...
  Component _buildSeparator() {
    ...
    final String separator = switch (props.separator) {
      ...
      BreadcrumbSeparatorStyle.chevron => '\u203A', // Single right-pointing angle
      BreadcrumbSeparatorStyle.arrow => '\u2192', // Right arrow
      BreadcrumbSeparatorStyle.dot => '\u2022', // Bullet
    };

    return dom.span(
      classes: 'codex-breadcrumb-separator',
      styles: const dom.Styles(raw: {
        'color': 'var(--muted-foreground)',
        'opacity': '0.5',
        'user-select': 'none',
