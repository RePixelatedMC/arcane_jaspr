...
/// Codex Slot Counter renderer (stateful with animation).
class CodexSlotCounter extends StatefulComponent {
  final SlotCounterProps props;

  const CodexSlotCounter(this.props, {super.key});

  @override
  State<CodexSlotCounter> createState() => _CodexSlotCounterState();
}

class _CodexSlotCounterState extends State<CodexSlotCounter> {
...
/// Codex Slot Counter Row renderer.
class CodexSlotCounterRow extends StatelessComponent {
  final SlotCounterRowProps props;

  const CodexSlotCounterRow(this.props, {super.key});

  @override
  Component build(BuildContext context) {
    return dom.div(
      classes: 'codex-slot-counter-row',
      styles: dom.Styles(raw: {
        'display': 'flex',
      ...
      [
        for (final counter in props.counters)
          CodexSlotCounter(counter),
      ],
    );
  }
}

/// Codex Slot Counter Card renderer.
class CodexSlotCounterCard extends StatelessComponent {
  final SlotCounterCardProps props;

  const CodexSlotCounterCard(this.props, {super.key});

  @override
  Component build(BuildContext context) {
    return dom.div(
      classes: 'codex-slot-counter-card',
      styles: dom.Styles(raw: {
        'display': 'inline-flex',
        'flex-direction': 'column',
        'align-items': 'center',
      ...
      [CodexSlotCounter(props.counter)],
    );
  }
}
