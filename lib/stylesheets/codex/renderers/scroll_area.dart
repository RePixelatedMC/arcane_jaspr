...
/// Codex Scroll Area renderer.
class CodexScrollArea extends StatelessComponent {
  final ScrollAreaProps props;

  const CodexScrollArea(this.props, {super.key});
  ...
  Component build(BuildContext context) {
    ...
    final scrollClassName = 'codex-scroll-area-${props.instanceId}';

    return dom.div(
      classes: 'codex-scroll-area $scrollClassName ${props.className ?? ""}',
  ...
  Component _buildScrollShadow(ScrollDirectionVariant direction, {required bool isStart}) {
    ...
    if (isVertical && !isHorizontal) {
      return dom.div(
        classes: 'codex-scroll-shadow ${isStart ? "top" : "bottom"}',
        styles: dom.Styles(raw: {
          'position': 'sticky',
          if (isStart) 'top': '0',
          if (!isStart) 'bottom': '0',
    ...
    if (isHorizontal) {
      return dom.div(
        classes: 'codex-scroll-shadow ${isStart ? "left" : "right"}',
        styles: dom.Styles(raw: {
          'position': 'sticky',
          if (isStart) 'left': '0',
          if (!isStart) 'right': '0',
...
/// Codex Virtual Scroll renderer.
class CodexVirtualScroll<T> extends StatelessComponent {
  final VirtualScrollProps<T> props;

  const CodexVirtualScroll(this.props, {super.key});
