...
class NeubrutalismScrollRail extends StatelessComponent {
  final ScrollRailProps props;

  const NeubrutalismScrollRail(this.props, {super.key});
  ...
  Component build(BuildContext context) {
    ...
    final String id =
        props.scrollPersistenceId ?? 'neubrutalism-scroll-rail-${props.position.name}';
    final String width = _width(props.size, props.width);

    return dom.div(
      id: id,
      classes: 'neubrutalism-scroll-rail',
      styles: dom.Styles(
        raw: {
          'position': 'sticky',
          'top': props.topOffset,
          ...
          'overflow-y': 'auto',
          'overflow-x': 'hidden',
          'background': props.background ?? 'var(--nb-paper, var(--card))',
          ...
          if (props.customScrollbar) ...{
            ...
            'scrollbar-color': 'var(--nb-accent, var(--primary)) transparent',
          },
        },
      ),
      [
...
class NeubrutalismScrollRailLayout extends StatelessComponent {
  final ScrollRailLayoutProps props;

  const NeubrutalismScrollRailLayout(this.props, {super.key});

  @override
  Component build(BuildContext context) {
    final Component rail = NeubrutalismScrollRail(
      ScrollRailProps(
        ...
        scrollPersistenceId: 'neubrutalism-main-rail',
        children: [props.rail],
      ),
    );

    final Component content = dom.div(
      classes: 'neubrutalism-scroll-rail-content',
      styles: dom.Styles(
        raw: {
          'flex': '1',
          'min-width': '0',
    ...
    return dom.div(
      classes: 'neubrutalism-scroll-rail-layout',
