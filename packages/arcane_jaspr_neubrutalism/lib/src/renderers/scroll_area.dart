...
class NeubrutalismScrollArea extends StatelessComponent {
  final ScrollAreaProps props;

  const NeubrutalismScrollArea(this.props, {super.key});
  ...
  Component build(BuildContext context) {
    ...
    final scrollClassName = 'neubrutalism-scroll-area-${props.instanceId}';

    return dom.div(
      classes: 'neubrutalism-scroll-area $scrollClassName ${props.className ?? ""}',
      styles: dom.Styles(
        raw: {
          'position': 'relative',
          ...
          'scrollbar-color':
              'var(--nb-ink, #000) var(--nb-paper-soft, #f1f1f1)',
  ...
  Component _buildScrollShadow(
    ...
    if (isVertical && !isHorizontal) {
      return dom.div(
        classes: 'neubrutalism-scroll-shadow ${isStart ? "top" : "bottom"}',
        styles: dom.Styles(
          raw: {
            'position': 'sticky',
            if (isStart) 'top': '0',
    ...
    if (isHorizontal) {
      return dom.div(
        classes: 'neubrutalism-scroll-shadow ${isStart ? "left" : "right"}',
        styles: dom.Styles(
          raw: {
            'position': 'sticky',
            if (isStart) 'left': '0',
...
class NeubrutalismVirtualScroll<T> extends StatelessComponent {
  final VirtualScrollProps<T> props;

  const NeubrutalismVirtualScroll(this.props, {super.key});
  ...
  Component build(BuildContext context) {
    ...
    return dom.div(
      classes: 'neubrutalism-virtual-scroll',
      styles: dom.Styles(
        raw: {
          'height': props.height,
          'overflow-y': 'auto',
